...
if [ -z "$URL" ]; then
    URL=$(yad --entry --title="Descargar video" --text="Pega el link del video o playlist:")
fi
...
if echo "$URL" | grep -qE "list=" && echo "$URL" | grep -qE "[?&]v="; then
    ...
    # Si el usuario cerró el diálogo
    [ $? -ne 0 ] && exit 0
    [ -z "$CHOICE" ] && exit 0

    # Limpiar valor de YAD
    CHOICE=$(echo "$CHOICE" | cut -d'|' -f1 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
...
if [ "$IS_PLAYLIST" = "1" ]; then
    ...
    tail -f --pid=$YTPID "$TMPLOG" | while read -r LINE; do
        ...
        --title="Descargando..." \
        --text="Descargando playlist: $PLAYLIST_TITLE" \
        --percentage=0 \
        --cancel-label="Cancelar" \
        --auto-close &
