...
if [ -z "$URL" ]; then
    URL=$(osascript -e 'text returned of (display dialog "Pega el link del video o playlist:" with title "Descargar video" default answer "" buttons {"Cancelar", "OK"} default button "OK")')
fi
...
if [ "$IS_PLAYLIST" = "1" ]; then
    ...
    /^[0-9]/ {
        ...
        for(i=1;i<=NF;i++){
            ...
            if($i ~ /^[0-9]+p[0-9]+$/){ split($i,m,"p"); res=m[1]"p"; fps=m[2]+0 }
    ...
    PROGPIPE=$(mktemp -u /tmp/ytprog-XXXX)
    mkfifo "$PROGPIPE"
    ...
    CURRENT_TITLE=""
    PERC=0
    { tail -f "$TMPLOG" & _TAILPID=$!; while kill -0 $YTPID 2>/dev/null; do sleep 0.3; done; sleep 0.5; kill $_TAILPID 2>/dev/null; } | while read -r LINE; do
...
BEFORE=$(find "$DOWNLOAD_DIR" -maxdepth 1 -type f -exec basename {} \; | sort)
...
if [ $ZENITY_EXIT -ne 0 ]; then
    ...
    AFTER_CANCEL=$(find "$DOWNLOAD_DIR" -maxdepth 1 -type f -exec basename {} \; | sort)
...
AFTER=$(find "$DOWNLOAD_DIR" -maxdepth 1 -type f -exec basename {} \; | sort)
