...
if [ -z "$URL" ]; then
    URL=$(zenity --entry --title="Reproducir video" --text="Pega el link del video:")
fi
...
[ -z "$VIDEO_OPTIONS" ] && zenity --error --text="No se encontraron formatos de video válidos." && exit 1
