...
if [ -z "$URL" ]; then
    URL=$(kdialog --title "Reproducir video" --inputbox "Pega el link del video:")
fi
...
[ -z "$VIDEO_OPTIONS" ] && kdialog --error "No se encontraron formatos de video válidos." && exit 1
...
CHOICE=$(kdialog --title "Reproducir: $TITLE" \
    --menu "Selecciona la resolución:" \
    "${MENU[@]}")
